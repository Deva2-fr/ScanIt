...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        # We don't raise here so the app can still start and serve 503 errors
...
# Always use explicit origins to avoid "Conflict with Allow-Credentials" browser errors
...
async def db_connection_exception_handler(request: Request, exc: OperationalError):
    ...
    return JSONResponse(
        ...
        content={
            ...
            "message": "Could not connect to the database. Please ensure the database service is running.",
            "detail": str(exc) if settings.debug else None
        }
    )
...
from .routers.billing_mock import router as billing_mock_router
