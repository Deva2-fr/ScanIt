...
class BrokenLinksResult(BaseModel):
    ...
    scanned_pages: List[str] = Field(default_factory=list, description="List of internal pages found during crawl")
    error: Optional[str] = None
...
class AnalyzeResponse(BaseModel):
    ...
    # Visual Regression
    visual_diff: Optional[VisualDiffResult] = Field(None, description="Visual regression results if applicable")

    # Battle Mode / Versus Mode
    versus_mode: bool = Field(False, description="True if this is a competitive comparison")
    winner: Optional[str] = Field(None, description="'target' if main URL wins, 'competitor' if competitor wins, 'draw' if equal")
    
    # Metadata
    screenshot_path: Optional[str] = Field(None, description="Path to the screenshot of the scan")
